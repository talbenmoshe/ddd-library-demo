...
function createGraph(dependencies: IDependency[]): DepGraph<string> {
  ...
  dependencies.forEach(dependency => {
    const pkgJson = sync({ cwd: dependency.path });
...
async function handleSequence(sequence: string[], timeout: number, entries: string[]) {
  ...
  }).map(pkg => path.join(pkg, '/dist/esm/src/index.js'));
  ...
  concurrently(sequence.map(file => `pnpm --prefix "${file}" start`), {
    cwd: process.cwd()
  });
...
start({
  entries: 'ddd-library-app ddd-library-server',
  timeout: '30000'
});